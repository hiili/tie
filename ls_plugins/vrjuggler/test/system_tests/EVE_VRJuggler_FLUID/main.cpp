...
{
  if( argc != 3 ) {
    cout << "Käyttö: " << argv[0] << " <juggler-conf> <fluid-conf>" << endl;
    exit(1);
  }
  
  cout << "Main starting... -> fluid init" << endl;
  
  Fluid::DeviceConfig::init(argv[2]);
  ...
  camObject.getLocator()->setLoc( makeVector3d( 0.0, -4.0, -3.0 ) );
