...
static TestWorld::CameraEntry createGLOWCamera( World & world )
{
  ...
  TestWorld::CameraEntry glowCam = { camera, camObject,
                                     0, camKeymap,
                                     TestWorld::CAM_MODE_ACTOR };
  return glowCam;
}
  ...
  public GLOWViewport
{
  ...
  {
    cout << "GLOWViewportEx::ActivateCamera() start\n";
    // deactivate the old camera
    if( currentCamera && currentCamera->mode == TestWorld::CAM_MODE_ACTOR ) {
    ...
    // select the new current camera default: glowCamera
    if( num == 0 ) {
      // glow camera
    ...
    cout << "GLOWViewportEx::ActivateCamera() finished\n";
  }
...
{
  if( argc != 2 ) {
    cout << "Usage: ./avatarbasic_<compilation mode> <fluid config file>\n";
    exit(1);
  }
  ...
  /* Create the world. */
  TestWorld world( window.events, inputHost );
